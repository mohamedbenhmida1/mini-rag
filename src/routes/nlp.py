...
async def index_project(request: Request, project_id: str, push_request: PushRequest):
    ...
    if not project:
        return JSONResponse(
            ...
            content={"signal": ResponseEnum.PROJECT_NOT_FOUND_ERROR.value},
        )
    ...
    while has_records:
        ...
        if not is_inserted:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"signal": ResponseEnum.INSERT_INTO_VECTORDB_ERROR.value},
            )
...
async def search_index(
    ...
    if not results:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"signal": ResponseEnum.VECTORDB_SEARCH_ERROR.value},
        )
...
async def answer_rag(request: Request, project_id: str, search_request: SearchRequest):
    ...
    if not answer:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"signal": ResponseEnum.RAG_ANSWER_ERROR.value},
        )
